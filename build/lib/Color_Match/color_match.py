...
hc_kB = 0.0143877735
data = np.loadtxt(os.path.join(os.path.dirname(__file__), 'lin2012xyz10e_fine_7sf.csv'), delimiter=',')
wavelengths = data[:,0] * 10**-9
s1 = data[:,1]
s2 = data[:,2]
s3 = data[:,3]
...
def planck_spectrum(t):
    ...
    return np.array([planck(l, t) for l in wavelengths])
...
def s_lookup(num, l):
    ...
    if num == 1:
        ...
        s = s3
    return s[np.argmin((wavelengths - l)**2.0)]
